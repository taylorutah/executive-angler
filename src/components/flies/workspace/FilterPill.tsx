    ...
    <Popover.Root>
      <Popover.Trigger asChild>
        <button
          type="button"
          className={[
            "inline-flex items-center gap-1.5 rounded-chip border px-2.5 py-1.5 text-xs font-medium transition-colors",
            activeCount > 0
        ...
        >
          {label}
          {activeCount > 0 && (
            <span className="inline-flex items-center justify-center min-w-[18px] h-[18px] px-1 rounded-chip bg-[var(--action)] text-white text-[10px] font-semibold tabular-nums">
              {activeCount}
            </span>
          )}
