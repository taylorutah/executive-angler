      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-2 mt-2 flex-wrap">
          {badges && badges.length > 0 && badges.map((badge) => (
            <span
              key={badge}
              className="px-2 py-0.5 text-xs font-medium bg-[var(--action)]/5 text-[var(--action)] rounded-chip"
            >
              {badge}
            </span>
          ))}
          {tags && tags.length > 0 && tags.map((tag) => (
            <span
              key={tag}
              className="px-2 py-0.5 text-xs bg-[var(--surface-card)] text-[var(--action)] rounded-chip"
            >
              {tag}
            </span>
          ))}
