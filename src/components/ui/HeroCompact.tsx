...
export default function HeroCompact({
  ...
  return (
    <>
      <div className="group relative flex items-stretch gap-3 sm:gap-4 bg-[var(--surface)] border border-[var(--border)] rounded-[var(--radius-card)] overflow-hidden">
        <button
          type="button"
          onClick={() => totalCount > 0 && setLightboxOpen(true)}
          className="relative shrink-0 w-[112px] sm:w-[200px] h-[112px] sm:h-[120px] bg-[var(--paper-deep)] focus:outline-none focus:ring-2 focus:ring-[var(--accent)] focus:ring-inset"
          aria-label={
            totalCount > 0
        ...
        >
          <SafeEntityImage
            ...
            className={`${imageContain ? "object-contain p-2" : "object-cover"} transition-transform duration-200 group-hover:scale-105`}
            sizes="(max-width: 640px) 112px, 200px"
            priority
          />
          {totalCount > 0 && (
            <div className="absolute bottom-1.5 left-1.5 sm:bottom-2 sm:left-2 flex items-center gap-1 sm:gap-1.5 px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-full bg-[var(--ink)] text-[var(--paper)] text-xs font-medium">
        ...
        <div className={`flex-1 min-w-0 py-2.5 sm:py-3 pr-3 sm:pr-4 flex flex-col justify-center ${children ? "pt-9 sm:pt-3" : ""}`}>
          <h1 className="font-heading text-lg sm:text-3xl font-semibold text-[var(--text-1)] tracking-[-0.01em] leading-[1.2] line-clamp-2 sm:truncate">
            {title}
          </h1>
          {subtitle && (
            <p className="mt-0.5 sm:mt-1 text-xs sm:text-base text-[var(--text-2)] truncate">
              {subtitle}
            </p>
          )}
          {visibleChips.length > 0 && (
            ...
            >
              {visibleChips.map((chip) => (
                <span
                  key={chip}
                  className="inline-flex shrink-0 items-center px-2 py-0.5 rounded-full bg-[var(--paper-deep)] border border-[var(--border)] text-xs text-[var(--text-2)] whitespace-nowrap"
                >
                  {chip}
                </span>
              ))}
        ...
        <div className="hidden sm:flex items-center pr-4">
          <button
            ...
            className="inline-flex items-center gap-2 px-3 py-2 rounded-[var(--radius-md)] border border-[var(--border-strong)] text-sm text-[var(--text-2)] hover:text-[var(--accent)] hover:border-[var(--accent)] transition-colors"
          >
