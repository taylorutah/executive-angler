  ...
  return (
    <div className="space-y-3">
      ...
      <button
        ...
        disabled={!!loading}
        className={`ea-focus-ring ${FOCUS_VISIBLE} w-full flex items-center justify-center gap-3 px-4 py-3 bg-[var(--accent-soft)] border border-[var(--accent-soft)] rounded-[var(--radius-md)] text-[var(--accent)] font-medium text-sm hover:bg-[color-mix(in_srgb,var(--accent)_12%,var(--accent-soft))] transition-colors duration-150 ease-standard disabled:opacity-50 motion-reduce:transition-none`}
      >
        {loading === "apple" ? (
          <span className="h-5 w-5 animate-spin rounded-full border-2 border-[var(--accent)]/40 border-t-[var(--accent)]" aria-hidden />
        ) : (
