"use client";

import { useState } from "react";
import Image from "next/image";
import { Images, Camera } from "lucide-react";
import PhotoLightbox from "./PhotoLightbox";
import type { ApprovedPhoto } from "@/lib/db/photos";

interface RiverHeroImageProps {
  heroImageUrl: string;
  heroImageAlt: string;
  heroImageCredit?: string;
  heroImageCreditUrl?: string;
  galleryPhotos: ApprovedPhoto[];
  children?: React.ReactNode; // admin editor slot
}

export default function RiverHeroImage({
  heroImageUrl,
  heroImageAlt,
  heroImageCredit,
  heroImageCreditUrl,
  galleryPhotos,
  children,
}: RiverHeroImageProps) {
  const [lightboxOpen, setLightboxOpen] = useState(false);

  const heroAsPhoto: ApprovedPhoto = {
    id: "hero",
    photoUrl: heroImageUrl,
    caption: heroImageAlt,
    submitterName: heroImageCredit || "Executive Angler",
    submittedAt: new Date().toISOString(),
  };

  const allPhotos = [heroAsPhoto, ...galleryPhotos];
  const totalCount = allPhotos.length;

  return (
    <>
      <div className="relative w-full overflow-hidden bg-[#0D1117]" style={{ height: "240px" }}>
        <Image
          src={heroImageUrl}
          alt={heroImageAlt}
          fill
          className="object-cover"
          priority
          sizes="100vw"
        />

        {/* Subtle bottom vignette for depth only — no text */}
        <div className="absolute inset-0 bg-gradient-to-b from-transparent via-transparent to-black/30 pointer-events-none" />

        {/* Gallery pill — bottom left */}
        {totalCount > 1 && (
          <button
            type="button"
            onClick={() => setLightboxOpen(true)}
            className="absolute bottom-3 left-3 z-10 inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-full bg-black/60 backdrop-blur-sm text-white/90 text-xs font-medium hover:bg-black/80 transition-colors"
            aria-label={`View ${totalCount} photos`}
          >
            <Images className="h-3.5 w-3.5" />
            {totalCount} photos
          </button>
        )}

        {/* Photo credit — bottom right */}
        {heroImageCredit && (
          <div className="absolute bottom-3 right-3 z-10">
            {heroImageCreditUrl ? (
              <a
                href={heroImageCreditUrl}
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-1 px-2 py-1 rounded bg-black/50 backdrop-blur-sm text-white/60 hover:text-white/90 transition-colors text-[10px] font-medium"
              >
                <Camera className="h-2.5 w-2.5" />
                {heroImageCredit}
              </a>
            ) : (
              <span className="inline-flex items-center gap-1 px-2 py-1 rounded bg-black/50 backdrop-blur-sm text-white/50 text-[10px] font-medium">
                <Camera className="h-2.5 w-2.5" />
                {heroImageCredit}
              </span>
            )}
          </div>
        )}

        {/* Admin editor slot — top right */}
        {children && (
          <div className="absolute top-3 right-3 z-20">{children}</div>
        )}
      </div>

      {lightboxOpen && (
        <PhotoLightbox
          photos={allPhotos}
          initialIndex={0}
          onClose={() => setLightboxOpen(false)}
        />
      )}
    </>
  );
}
