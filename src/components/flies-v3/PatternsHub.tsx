...
import { Search, Heart, Wrench, Sparkles } from "lucide-react";
import type { PatternsHubRow } from "@/lib/db/fly-model";
import { summarizeVersion } from "./summarize-version";

type TabKey = "all" | "created" | "favorites" | "tie-next";
...
export default function PatternsHub({
  ...
  const isCustom = (r: PatternsHubRow) =>
    viewerUserId !== null &&
    (r.fly as { submitted_by_user_id?: string | null }).submitted_by_user_id === viewerUserId;
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((r) => {
      ...
      if (tab === "created" && !isCustom(r)) return false;
  ...
  return (
    <section>
      <div role="tablist" aria-label="Patterns view" className="flex items-center gap-1 mb-4 border-b border-[var(--color-border,#e5e7eb)] dark:border-[#21262D]">
        ...
        <TabBtn active={tab === "created"}    onClick={() => setTab("created")}    label={`Created by me ${totalCreated}`} />
      ...
      {filtered.length === 0 ? (
        <p className="text-sm text-[var(--color-text-muted)] py-12 text-center">
          ...
          {tab === "created"   && "You haven't created any flies yet. Clone a canonical pattern or start a new one."}
        ...
        <ul className="space-y-2">
          {filtered.map((r) => {
            ...
            // Private + pending custom flies don't have routable /flies/{slug}
            // URLs — they live under the user's angler profile. Approved flies
            // (even user-submitted ones) route through the canonical library.
            const isPrivateRoute =
              custom && (flyStatus === "private" || flyStatus === "pending");
            const href =
            ...
            const versionText =
              r.versions.length === 0
                ? "No versions yet"
                : `${r.versions.length} version${r.versions.length === 1 ? "" : "s"}: ${r.versions
                    .slice(0, 3)
                    .map(summarizeVersion)
                    .join(", ")}${r.versions.length > 3 ? ` +${r.versions.length - 3}` : ""}`;
            return (
            <li key={r.fly.id}>
              <Link
                href={href}
