...
/**
 * Homepage uses --scrim-light so the photograph stays at full strength
 * (brand bible §5.3). The 11px eyebrow sits on an opaque Vellum chip
 * (slate on vellum is 5.03:1) and needs no scrim. White type sits on a
 * text-local `.hero-text-panel`, never a heavier full-frame wash.
 * Display type is tall; content is in-flow. Do not use `.prose` here —
...
export default function HomeHero({ riverCount, cfs }: Props) {
  ...
  return (
    ...
    >
      ...
      />
      <div className="hero-overlay-home absolute inset-0" aria-hidden />
      ...
      <div className="relative z-10 flex min-h-[70svh] flex-col justify-end sm:min-h-[85vh]">
        <div className="mx-auto w-full max-w-7xl px-4 pb-12 pt-[22vh] sm:px-6 sm:pb-16 lg:px-8">
          ...
          <div className="relative max-w-5xl">
            <div
              className="hero-text-panel pointer-events-none absolute -inset-x-4 -top-3 -bottom-6 sm:-inset-x-6"
              aria-hidden
            />
            <h1
              className="relative font-heading font-bold tracking-tight text-white drop-shadow-lg"
            ...
            <form
              ...
              method="get"
              role="search"
              className="relative mt-8 flex w-full max-w-[640px] items-center gap-2 rounded-lg border border-[var(--rule)] bg-white px-4 py-3 focus-within:border-[var(--ink)]"
            >
            ...
            <Link
              href="/rivers"
              className="relative mt-4 inline-block text-[14px] text-white underline-offset-4 transition-colors hover:underline"
            >
