...
export default function MaterialsQueueClient({ pending: initial }: Props) {
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      <h1 className="font-display text-2xl sm:text-3xl font-semibold text-[var(--text-1)] mb-2">
      ...
      {error && (
        <div className="mb-4 p-3 rounded-[var(--radius-md)] border border-[var(--danger)]/30 bg-[var(--danger)]/10 text-[var(--danger)] text-sm">
          {error}
        </div>
      )}
      ...
      {items.length === 0 ? (
        ...
        <div className="space-y-3">
          {items.map(item => {
            ...
            return (
              <div key={item.id} className="ea-card p-4">
                <div className="flex items-start justify-between gap-3 mb-2">
                  <div className="flex-1 min-w-0">
                    {isEditing ? (
                      <input
                        ...
                        className="ea-input font-semibold"
                      />
                    ) : (
                      <p className="text-[var(--text-1)] font-semibold">{merged.name}</p>
                    )}
                  ...
                  <div className="flex gap-2 flex-shrink-0">
                    <button
                      ...
                      className="p-1.5 rounded-[var(--radius-md)] border border-[var(--border)] text-[var(--text-2)] hover:text-[var(--text-1)] hover:border-[var(--border-strong)] transition-colors duration-150 ease-standard"
                    ...
                    <button
                      ...
                      disabled={isBusy}
                      className="p-1.5 rounded-[var(--radius-md)] border border-[var(--danger)]/30 text-[var(--danger)] hover:bg-[var(--danger)]/10 transition-colors duration-150 ease-standard disabled:opacity-50"
                      title="Reject (delete)"
                    >
                      <X size={14} />
                    </button>
                ...
                {(merged.description || isEditing) && (
                  <div className="mt-2">
                    <label className="block text-xs font-medium text-[var(--text-3)] mb-1">Description</label>
                    {isEditing ? (
                      <textarea
                      ...
                      <p className="text-[var(--text-2)] text-xs">{merged.description}</p>
                    )}
                  </div>
                )}
              </div>
...
function Field({ label, value, editing, onChange }: { label: string; value: string; editing: boolean; onChange: (v: string) => void }) {
  return (
    <div>
      <label className="block text-xs font-medium text-[var(--text-3)] mb-0.5">{label}</label>
      {editing ? (
        <input
          type="text"
          value={value}
        ...
        <p className="text-[var(--text-1)]">{value || '—'}</p>
      )}
    </div>
  );
}
  ...
  return (
    <div>
      ...
      {editing ? (
        ...
        <p className="text-[var(--text-1)]">{value.length ? value.join(', ') : '—'}</p>
      )}
    </div>
  );
}
