  ...
  const meta = stats.length === 0
    ...
    : (
      <span className="inline-flex flex-wrap items-baseline gap-x-2">
        <span><span className="text-[#F0F6FC] font-semibold">{stats.length}</span> rivers</span>
    ...
    <>
      ...
      {stats.length === 0 ? (
        ...
        <div className="bg-[#161B22] border border-[#21262D] rounded-xl divide-y divide-[#21262D]">
          {stats.map((s) => (
            <RiverRow key={s.river_name} stats={s} isPremium={isPremium} />
          ))}
        </div>
      )}
    </>
...
function RiverRow({ stats: s, isPremium }: { stats: RiverStats; isPremium: boolean }) {
  ...
  return (
    <Link
      href={href}
      className="group block px-4 py-3.5 hover:bg-[#1F2630] transition-colors first:rounded-t-xl last:rounded-b-xl"
    >
      <div className="flex items-baseline justify-between gap-3">
        ...
        <div className="flex items-center gap-2 shrink-0">
          {AWARDS_VISIBLE && isPremium && s.awards.length > 0 && (
            <span className="font-mono text-[10px] text-[#E8923A] tracking-[0.04em]">
              ★ {s.awards.length}
            </span>
          )}
          <ChevronRight className="h-3.5 w-3.5 text-[#3a4150] group-hover:text-[#A8B2BD] transition-colors" />
        </div>
      </div>
      <div className="mt-1 flex flex-wrap items-baseline gap-x-2 gap-y-0.5 font-mono text-[12px] text-[#6E7681] tabular-nums">
        <span>
          <span className="text-[#A8B2BD]">{s.total_sessions}</span>{' '}
          {s.total_sessions === 1 ? 'session' : 'sessions'}
        </span>
        <span className="text-[#3a4150]">·</span>
        <span>
          <span className="text-[#A8B2BD]">{s.total_fish}</span> fish
        </span>
        <span className="text-[#3a4150]">·</span>
        <span>
          <span className="text-[#A8B2BD]">{s.avg_fish_per_session.toFixed(1)}</span>/s
        </span>
        {s.biggest_fish ? (
          <>
            <span className="text-[#3a4150]">·</span>
            <span>
              <span className="text-[#A8B2BD]">{s.biggest_fish}&quot;</span> pb
            </span>
          </>
        ) : null}
        {s.species_caught.length > 0 ? (
          <>
            <span className="text-[#3a4150]">·</span>
            <span>
              <span className="text-[#A8B2BD]">{s.species_caught.length}</span>{' '}
              {s.species_caught.length === 1 ? 'species' : 'species'}
            </span>
          </>
        ) : null}
        {s.favorite_fly ? (
          <>
            <span className="text-[#3a4150]">·</span>
            <span className="lowercase">{s.favorite_fly}</span>
          </>
        ) : null}
        <span className="text-[#3a4150]">·</span>
