...
const ENTITY_TYPES = [
  ...
  { type: "fly_shop", label: "Fly Shop", icon: "🏪", description: "Local fly shops and outfitters", color: COPPER_700 },
  ...
  { type: "photo_update", label: "Update a Listing Photo", icon: "📷", description: "Submit a hero photo for an existing fly shop, lodge, guide, or river", color: COPPER_700 },
];
    ...
    <div className="min-h-screen bg-[var(--surface-page)]">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 pb-16">
        ...
        <div className="text-center mb-10">
          <h1 className="font-serif text-3xl text-[var(--text-primary)] mb-3">Contribute to Executive Angler</h1>
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          {ENTITY_TYPES.map(({ type, label, icon, description, color }) => (
            <Link
              ...
              className="group p-5 bg-[var(--surface-raised)] border border-[var(--border-rule)] rounded-xl hover:border-opacity-60 transition-all"
              style={{ ["--hover-color" as string]: color }}
            >
              <div className="flex items-start gap-4">
                <span className="text-3xl">{icon}</span>
                <div>
                  <h2 className="text-base font-bold text-[var(--text-primary)] group-hover:text-[var(--action)] transition-colors">{label}</h2>
                  <p className="text-xs text-[var(--text-body)] mt-1">{description}</p>
                </div>
              </div>
            </Link>
          ))}
        ...
        {/* Info section */}
        <div className="mt-10 bg-[var(--surface-raised)] border border-[var(--border-rule)] rounded-xl p-6">
          ...
          <ol className="space-y-3 text-sm text-[var(--text-body)]">
            <li className="flex gap-3">
              <span className="flex items-center justify-center w-6 h-6 rounded-full bg-[var(--action)]/15 text-[var(--action)] text-xs font-bold shrink-0">1</span>
              <span>Choose what you want to add and fill in the details. A high-quality photo is strongly recommended.</span>
            </li>
            <li className="flex gap-3">
              <span className="flex items-center justify-center w-6 h-6 rounded-full bg-[var(--action)]/15 text-[var(--action)] text-xs font-bold shrink-0">2</span>
              <span>Submit for review. You can save as a draft and come back later from your Account page.</span>
            </li>
            <li className="flex gap-3">
              <span className="flex items-center justify-center w-6 h-6 rounded-full bg-[var(--action)]/15 text-[var(--action)] text-xs font-bold shrink-0">3</span>
        ...
        <div className="mt-6 text-center">
          <Link href="/account#submissions" className="text-sm text-[var(--action)] hover:underline">
            View my submissions →
          </Link>
        </div>
      </div>
