/**
 * v2 Box detail — variant table scoped to one fly_box.
 *
 * Reuses VariantTable but defaults to read-only stock context (the variants
 * here are the user's own; clicking a row jumps to the pattern's v2 detail).
 */
...
export default async function BoxDetailV2({ params }: Props) {
  ...
  if (!user) redirect(`/login?redirect=/flies/boxes/v2/${id}`);
      ...
      <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-6">
        ...
        {variants.length === 0 && (
          <p className="text-xs text-[#6E7681] text-center mt-4">
            ...
            <Link href="/flies/v2/perdigon" className="text-[#0BA5C7] hover:text-[#E8923A]">
              Perdigon
            </Link>
