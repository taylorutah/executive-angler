// Apply 20260522_river_fly_pulse_by_session_date.sql via the Supabase pooler.
...
const sql = readFileSync(
  "/Users/taylorwarnick/My Sites-Apps/personal/executive-angler/supabase/migrations/20260522_river_fly_pulse_by_session_date.sql",
  "utf8",
);
...
async function tryApply(host) {
  ...
  console.log(`[VERIFY] river_fly_pulse('provo-river') → ${check.rows.length} rows`);
  for (const row of check.rows) {
    console.log(`  - ${row.fly_name} (sizes: ${(row.sizes || []).join(", ") || "—"})`);
  }
...
if (!applied) {
  ...
  console.error(
    "  supabase/migrations/20260522_river_fly_pulse_by_session_date.sql",
  );
  process.exit(2);
}
