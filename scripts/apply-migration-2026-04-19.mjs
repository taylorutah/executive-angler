// One-off migration applier — applies 20260419_is_demo_flag.sql to Supabase.
...
const sql = readFileSync(
  "/Users/taylorwarnick/My Sites-Apps/personal/executive-angler/supabase/migrations/20260419_is_demo_flag.sql",
  "utf8"
);
...
// Broader region sweep — "Tenant or user not found" from pooler usually means
// wrong region. Supabase has projects across many AWS regions.
...
async function tryApply(host) {
  ...
  const check = await client.query(
    "SELECT column_name FROM information_schema.columns WHERE table_name IN ('fishing_sessions','catches') AND column_name = 'is_demo'"
  );
  console.log("[VERIFY] is_demo columns present:", check.rows);
  await client.end();
}
